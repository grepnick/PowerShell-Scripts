...
$ConfigFilePath = "C:\ProgramData\NinjaRMMAgent\policy\agent.config.trayicon.json"
$searchString1 = "Company Name"

#$URL = 'https://asdf.msi'

#if (Test-Path $ConfigFilePath) {
#    $ConfigFileContent = Get-Content -Path $ConfigFilePath -Raw
#    if ($ConfigFileContent -match [regex]::Escape($searchString1)) {
#        Write-Output "Aimbridge agent detected."
#    } else { 
#      Write-Output "Aimbridge agent not detected, exiting." 
#      exit
#    }
#}
...
function Install-MSI {
  ...
  if (Get-NinjaInstallStatus) {
    Write-Output 'Cannot continue as NinjaOne is already installed.'
    Exit 0
  }
  ...
  if (!($FileDownload)) {
    Write-Output 'Failed to download file. Exiting.'
    Exit 1
  }
  
  Install-MSI -File $Destination
  
  if (!(Get-NinjaInstallStatus)) {
    Write-Output 'Failed to install NinjaOne agent.'
    Exit 1
  }
  
  Write-Output 'Successfully installed NinjaOne agent.'
  Exit 0
